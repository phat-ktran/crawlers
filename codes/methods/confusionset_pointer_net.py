...
# ----------------------------
def masked_softmax(logits: torch.Tensor, mask: torch.Tensor, dim: int = -1, eps: float = 1e-9):
    """
    logits: (..., V)
...
class ConfusionPointerNet(nn.Module):
    def __init__(
        ...
        self.encoder = nn.LSTM(input_size=embed_dim, hidden_size=enc_hidden, num_layers=1,
        ...
        # Wi maps concatenated vector -> (n+1) logits in forward (we will adapt size)
        # Wi will be created on the fly per batch because its output dim depends on n (input length)
        # So we'll create a small linear that maps to a hidden that we then multiply with learned vector
        # For simplicity: Wi -> produce a score per input position by dot-product with an encoding of positions.
        # We'll implement a parameter vector `pos_v` used to dot with position embeddings.
        # But simpler: we'll produce position logits by linear of size dec_hidden -> n (done per sample).
        # To keep implementation simple, we produce position logits via: pos_logits = LocMatrix @ (A * [Cj]) where LocMatrix is identity.
        # Implementation detail below in forward().

        # Loc embedding isn't necessary as Loc is one-hot; instead we will concat one-hot Loc and Cj and project
        self.pointer_proj = nn.Linear(dec_hidden + 1, 128)
        self.pointer_score = nn.Linear(128, 1)
    ...
    def _init_weights(self):
        ...
        for l in [self.W1, self.W2, self.v_att, self.combine, self.vocab_proj, self.Wg, self.pointer_proj, self.pointer_score]:
            if hasattr(l, 'weight'):
    ...
    def attention(self, dec_h: torch.Tensor, enc_hs: torch.Tensor, enc_mask: torch.Tensor):
        """
        dec_h: (B, dec_hidden)
        enc_hs: (B, n, 2*enc_hidden)
    ...
    def forward(
        ...
        confusionset_mask: torch.Tens.edusor,
        ...
        prev_input_ids = torch.full((B,), self.pad_idx, dtype=torch.long, device=device)  # start with PAD token embedding
        results_pred = []
        pointer_logits_all = []
        vocab_logits_all = []
        loss_pointer = 0.0
        loss_gen = 0.0
        ce_loss = nn.CrossEntropyLoss(reduction='sum', ignore_index=-100)  # we'll manage masking

        for j in range(n):
            # prepare input embedding
            emb_prev = self.embed(prev_input_ids)  # (B, E)
            dec_h, dec_c = self.decoder_cell(emb_prev, (dec_h, dec_c))  # each (B, dec_hidden)
            # attention
            context, alpha = self.attention(dec_h, enc_hs, src_mask)  # context: (B, 2*enc_hidden)

            # combine to produce Cj (paper: Cj = tanh(W (ht_j ; htj'))
            combined = torch.tanh(self.combine(torch.cat([dec_h, context], dim=-1)))  # (B, dec_hidden)
            ...
            pointer_input = torch.cat([Cj_exp, Locj_unsq], dim=-1)  # (B, n, dec_hidden+1)
            pointer_hidden = torch.tanh(self.pointer_proj(pointer_input))  # (B, n, 128)
            pos_scores = self.pointer_score(pointer_hidden).squeeze(-1)  # (B, n) scores for each input position
            # sentinel score: produce an extra logit for sentinel using a small MLP on combined
            sentinel_logit = torch.tanh(self.Wg(combined)).sum(dim=-1, keepdim=True)  # (B, 1)
            pointer_logits = torch.cat([pos_scores, sentinel_logit], dim=-1)  # (B, n+1)
            # mask positions that are padding in src_mask: set to -inf
            pad_mask = torch.cat([src_mask, torch.ones(B, 1, device=device)], dim=1)  # (B, n+1) allow sentinel
            ...
            if tgt_ids is not None:
                ...
                labels_pos = torch.full((B,), n, dtype=torch.long, device=device)  # default sentinel
                tgt_j = tgt_ids[:, j]  # (B,)
                ...
                if valid_mask.any():
                    pointer_loss_j = F.cross_entropy(pointer_logits[valid_mask], labels_pos[valid_mask], reduction='sum')
                else:
                ...
                if generate_mask.any():
                    ...
                    neg_inf = -1e9
                    masked_logits = gen_logits.masked_fill(gen_mask == 0, neg_inf)  # (G, V)
                    # if some row has all zeros mask (shouldn't), fallback to unmasked logits
                    # compute CE
                    gen_loss_j = F.cross_entropy(masked_logits, tgt_gen, reduction='sum')
                else:
            ...
            with torch.no_grad():
                ...
                next_ids = torch.full((B,), self.pad_idx, dtype=torch.long, device=device)
...
if __name__ == "__main__":
    ...
    vocab = ['<pad>', '这', '使', '我', '永', '生', '难', '望', '忘', '汪', '圣', '晚']  # V = 12
    ...
    def build_confusion_mask(batch_src_ids: torch.Tensor, vocab_size: int, conf_map: Dict[int, List[int]]):
        # batch_src_ids: (B, n)
    ...
    src_ids = torch.tensor([[stoi[c] for c in src_sentence]], dtype=torch.long)  # (1, 7)
    ...
    model = ConfusionPointerNet(vocab_size=V, embed_dim=64, enc_hidden=128, dec_hidden=128, attn_dim=128, pad_idx=pad_idx)
